...
namespace SwissTransport
{
    public class TransportHandler : ITransportHandler
    {
        ...
        public Connections GetConnections(string fromStation, string toStation, int limit)
        {
            ...
            var request = CreateWebRequest("http://transport.opendata.ch/v1/connections?from=" + fromStation + "&to=" + toStation + "&limit=" + limit); // added a limit parameter to specify amount of connections returned by the API
            var response = request.GetResponse();
            var responseStream = response.GetResponseStream();
